...
export default function AuthenticationPage() {
  ...
  const handleEmailLogin = async (e: React.FormEvent) => {
    ...
    try {
      // Mocking the backend for the prototype
      setTimeout(() => {
        const mockUser = {
          id: 1,
          email: email,
  ...
  const handleMockSocialLogin = (provider: string) => {
    ...
    setTimeout(() => {
      const mockUser = {
        ...
        first_name: provider === 'google' ? 'Google' : 'Apple',
      };
      login(`mock_token_${provider}`, mockUser);
      window.location.href = "/dashboard";
    }, 1000);
  };
  ...
  return (
    <div className="relative min-h-screen flex overflow-hidden font-sans">
      ...
      <div className="relative z-10 w-full lg:w-1/2 flex items-center justify-center p-8 sm:p-12 bg-card/60 backdrop-blur-3xl border-l border-white/20">
        <div className="w-full max-w-md">
          ...
          <div className="flex items-center gap-5 mb-10">
            <img 
              ...
              alt="Healthy Meal" 
              className="w-16 h-16 rounded-2xl object-cover shadow-lg border border-gray-200 dark:border-gray-700 shrink-0" 
            />
            <div className="text-left">
              <h1 className="text-4xl font-black tracking-tight">
                <span className="text-slate-900">Smart</span><span className="text-emerald-600">Meal</span> Planner
              </h1>
              ...
              >
                <div className="space-y-4 mb-10">
                  <button 
                    ...
                    className="w-full flex items-center justify-center gap-4 bg-card border border-slate-200 text-slate-700 px-4 py-4 rounded-2xl font-bold text-sm hover:scale-[1.02] active:scale-[0.98] transition-all shadow-sm group"
                  >
                ...
                <div className="flex p-1.5 bg-slate-100/80 rounded-2xl mb-8">
                  <button onClick={() => { playHapticTap(); setActiveTab("EMAIL"); }} className={`flex-1 py-3 text-xs font-black uppercase tracking-widest rounded-xl transition-all ${activeTab === "EMAIL" ? "bg-card text-emerald-600 shadow-sm" : "text-slate-500 hover:text-slate-800"}`}>Email</button>
                  <button onClick={() => { playHapticTap(); setActiveTab("PHONE"); }} className={`flex-1 py-3 text-xs font-black uppercase tracking-widest rounded-xl transition-all ${activeTab === "PHONE" ? "bg-card text-emerald-600 shadow-sm" : "text-slate-500 hover:text-slate-800"}`}>Phone</button>
                </div>
                ...
                {activeTab === "EMAIL" ? (
                  <form className="space-y-5" onSubmit={handleEmailLogin}>
                    <div className="space-y-1.5">
                        ...
                        <input type="email" value={email} onChange={(e) => setEmail(e.target.value)} placeholder="athlete@smartmeal.ai" required className="w-full px-5 py-4 bg-card/80 border border-slate-200 rounded-2xl text-sm focus:border-emerald-500 outline-none transition-all font-medium placeholder:text-slate-300 shadow-inner" />
                    </div>
                    <div className="space-y-1.5">
                        <label className="text-[10px] uppercase font-black text-slate-400 tracking-widest ml-1">Keycode</label>
                        <input type="password" value={password} onChange={(e) => setPassword(e.target.value)} placeholder="••••••••" required className="w-full px-5 py-4 bg-card/80 border border-slate-200 rounded-2xl text-sm focus:border-emerald-500 outline-none transition-all font-medium placeholder:text-slate-300 shadow-inner" />
                    </div>
                ...
                /* PHONE FORM */
                  <form className="space-y-5" onSubmit={handleRequestOTP}>
                    <div className="space-y-1.5">
                        ...
                        <input type="tel" value={phone} onChange={(e) => setPhone(e.target.value)} placeholder="+91 98765 43210" required className="w-full px-5 py-4 bg-card/80 border border-slate-200 rounded-2xl text-sm focus:border-emerald-500 outline-none transition-all font-medium placeholder:text-slate-300 shadow-inner" />
                    </div>
              ...
              >
                ...
                <form onSubmit={handleVerifyOTP} className="space-y-6">
                  <input 
                    ...
                    maxLength={6}
                    required 
                    className="w-full px-5 py-6 bg-card border-2 border-slate-100 rounded-2xl text-4xl tracking-widest text-center font-black focus:border-emerald-500 outline-none transition-all shadow-xl text-emerald-600" 
                  />
        ...
        <AnimatePresence>
          {isChatOpen && (
            <motion.div 
              ...
              className="absolute bottom-24 right-0 w-85 bg-card/95 backdrop-blur-2xl rounded-[32px] shadow-2xl border border-white/50 overflow-hidden mb-4 flex flex-col"
              style={{ height: '480px' }}
            >
              <div className="gradient-emerald p-6 text-white flex items-center justify-between">
                ...
                <button onClick={() => setIsChatOpen(false)} className="w-10 h-10 flex items-center justify-center rounded-2xl hover:bg-card/20 transition-all font-black">✕</button>
              </div>
                ...
                {messages.map((m, i) => (
                    <div key={i} className={`flex ${m.sender === 'user' ? 'justify-end' : 'justify-start'}`}>
                        <div className={`max-w-[85%] p-4 rounded-2xl text-sm font-medium ${
                            ...
                            : 'bg-card text-slate-700 rounded-bl-none border border-slate-100 shadow-sm'
                        }`}>
                            {m.text}
                        </div>
                    </div>
              ...
              <form onSubmit={handleSendMessage} className="p-4 bg-card border-t border-slate-100 flex gap-2">
                <input 
                    type="text" 
                    value={chatInput} 
                    onChange={(e) => setChatInput(e.target.value)}
                    placeholder="Ask Neural Hub..." 
                    className="flex-1 px-5 py-3 bg-slate-100/50 rounded-2xl text-xs font-bold outline-none focus:bg-card focus:ring-2 ring-emerald-500/20" 
                />
